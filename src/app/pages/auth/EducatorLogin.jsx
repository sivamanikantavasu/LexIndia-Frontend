    ...
    try {
      ...
      if (signInError) {
        if (signInError.message.includes('Email not confirmed')) {
          throw new Error('Account exists but email is not confirmed. Please check your inbox or confirm it in the Supabase dashboard.');
        }
        throw signInError;
      }
      ...
      if (profileError || profile?.role !== 'educator') {
        ...
        throw new Error('Access denied. You do not have educator privileges.');
      }

      // Successful login - navigate to educator dashboard
      navigate('/educator');
    } catch (err) {
        ...
        <div className="glass-white rounded-2xl p-8 md:p-10 shadow-2xl border border-[#FF9933]/20">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-gradient-to-r from-[#FF9933] to-[#FFB366] text-white rounded-lg hover:shadow-lg transition-all hover-lift disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center"
            >
              {loading ? (
